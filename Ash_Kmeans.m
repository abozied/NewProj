%% (C) Copyright 2012. All rights reserved. Sotiris L Karavarsamis.Contact the author at <sokar@aiia.csd.auth.gr>
%  This is my implementation on the k-means algorithm straight from the pseudocode description of the very same algorithm on the book
%  'Introduction to Information Retrieval' by Manning, Schutze and Raghavan.
function [C, I, iter] = Ash_Kmeans(X, K, maxIter, TOL)
...
while 1
    ...
    for n=1:vectors_num
        ...
        %minVal = euclidean(X(n,:), C(minIdx,:));
        minVal = Cossim (X(n,:), C(minIdx,:));
        for j=1:K
            %dist = norm(C(j,:) - X(n,:), 1);
            %dist = euclidean(C(j,:), X(n,:));
            dist =Cossim( X(n,:) , C(j,:));
            if dist < minVal
                minIdx = j;
                minVal = dist;
    ...
    for idx=1:vectors_num
        ...
        RSS_error = RSS_error + euclidean(X(idx, :), C(I(idx),:));
    end
